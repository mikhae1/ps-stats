...
Wathcing running process restarts including its children.
Cache file is used to store statistics. Invalidation is set by CACHE_TTL.
Script needed to be run by external program (Zabbix agent, cron).
...
CACHE_TTL = ENV.has_key?('CACHE_TTL') ? Integer(ENV['CACHE_TTL']) : 55
...
def main
  ...
  optparse = OptionParser.new do |opts|
    ...
    opts.on("-S", "--sub-parent", "Main process is the first child of PID") do |p|
      options[:subparent] = p
    end
  ...
  if options[:pid]
    ...
    p_pid = `systemctl show -p MainPID #{options[:systemd]} | grep -o '[0-9]*'`
  ...
  begin
    p_pid = `pgrep -P #{p_pid}` if options[:subparent]
    p_pid = Integer(p_pid)
